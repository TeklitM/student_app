...
class EditStudentPage extends StatelessWidget {
   EditStudentPage({super.key,Student? student}){
     Get.delete<EditStudentController>();
    Get.put(EditStudentController(student));
  }

  @override
  Widget build(BuildContext context) {
      return GetBuilder<EditStudentController>(builder: (controller) {
      return Scaffold(
          appBar: AppBar(
                ...
                children: [
                  ...
                  Stack(
                    children: [
                      CircleAvatar(
                        radius: 80,
                        foregroundImage:controller.file !=null ? FileImage(controller.file!)as ImageProvider : controller.profileImage == null ?
                        const AssetImage('assets/images/user.png')
                            :NetworkImage("${Keys.baseUrl}/${controller.profileImage}") as ImageProvider ,
                      ),
                      Positioned(
                          bottom: 0.0,
                          right: 0.0,
                  ...
                  PrimaryButton(
                      ...
                      onPressed: () async {
                        if (controller.validateForms()) {
                          await controller.editNewStudent();
                        }
                      }),
                ],
              ),
